...
import com.example.mvvmlibrary.lib_mvvm.base.livedata.ipc.encode.IEncoder;
import com.example.mvvmlibrary.lib_mvvm.base.livedata.ipc.encode.ValueEncoder;
import com.example.mvvmlibrary.lib_mvvm.base.livedata.ipc.json.GsonConverter;
import com.example.mvvmlibrary.lib_mvvm.base.livedata.ipc.json.JsonConverter;
import com.example.mvvmlibrary.lib_mvvm.base.livedata.ipc.receiver.LebIpcReceiver;
import com.example.mvvmlibrary.lib_mvvm.base.livedata.logger.DefaultLogger;
import com.example.mvvmlibrary.lib_mvvm.base.livedata.logger.Logger;
import com.example.mvvmlibrary.lib_mvvm.base.livedata.logger.LoggerManager;
...
public final class LiveEventBusCore {
     ...
     * 存放LiveEvent
     */
    private final ConcurrentHashMap<String, LiveEvent<Object>> bus;
    ...
    public synchronized <T> Observable<T> with(String eventKey,String tag, Class<T> type) {
...
//            if (ContextProvider.get().getContext() != null) {
...
//        }
        ...
        @MainThread
        private void observeStickyInternal(@NonNull LifecycleOwner owner, @NonNull
                Observer<T> observer) {
            ObserverWrapper<T> observerWrapper = new ObserverWrapper<>(observer);
            liveData.observe(owner, observerWrapper);
            logger.log(
        ...
        private void observeStickyForeverInternal(@NonNull Observer<T> observer) {
            ...
            liveData.observeForever(observerWrapper);
            logger.log(
                Level.INFO, "observe sticky forever observer: " + observerWrapper + "(" + observer + ")"
                    + " with key: " + key);
        }
    ...
    class InnerConsole {
        ...
        String getBaseInfo() {
            ...
            sb.append("lifecycleObserverAlwaysActive: ").append(lifecycleObserverAlwaysActive).append("\n")
                   ...
                   // .append("Application: ").append(ContextProvider.get().getContext()).append("\n");
            return sb.toString();
        }
